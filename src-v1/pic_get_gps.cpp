...
static double pipeline_delay_s = 0.00; // 可调的固定管线延迟（秒），用于将帧到达时间回溯到曝光中点 如开始所说后面没有任何对它进行修改的逻辑
...
double p11, p12, p21, p22;//卡尔曼滤波有两大噪声：过程噪声(飞机本身有多不稳定)和测量噪声(GPS本身测量上的误差) 分别用Q矩阵和R矩阵描述 均为二维矩阵
...
{
	buf.emplace_back(TimedVal{ t, v });
	while (buf.size() > BUF_MAX) buf.pop_front();//这个函数将时间和速度两个量储存 超过最大容量的时候就自动删除最老的一条
}
...
// GStreamer管道生成函数 通过Invida jetson进行摄像头采集
...
void imu_cb(const sensor_msgs::Imu::ConstPtr& msg)
{
	...
	g2 = 2.0f * (imu.orientation.w * imu.orientation.x + imu.orientation.y * imu.orientation.z);
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < array_len; i++) 
	{
		lat_array_kf[i] = global_pos.latitude;
	...
	while (ros::ok())
	{
		...
		if (dis < dis_lim && plane_alt > 10)
		{
			...
			Mat cell_raw = Mat::zeros(Size(150, 8), CV_8UC3);

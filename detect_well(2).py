...
import sys
sys.path.insert(0, '/home/tx2/Wintter/devel/lib/python3/dist-packages')
...
def send_gps_via_ros(lat, lon):
    ...
    global gps_pub, yoloresult_pub, running
...
def crop_save_thread():
    """异步写裁剪图片到磁盘，避免阻塞推理线程"""
    while running or crop_save_queue:
        item = None
        with crop_save_lock:
            if crop_save_queue:
